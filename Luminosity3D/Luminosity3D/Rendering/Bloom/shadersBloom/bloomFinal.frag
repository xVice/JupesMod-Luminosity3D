...
vec3 FilmGrain(vec3 inputColor)
{
    float filmGrainAmount = film_grain;
    const float PI = 3.14159265359;

    // Generate a random intensity based on screen coordinates and elapsed time
    float randomIntensity = fract(10000.0 * sin((TexCoords.x * gl_FragCoord.x) + (TexCoords.y * gl_FragCoord.y) * elapsedTime * PI));

    // Scale the random intensity by the specified film grain amount
    float scaledIntensity = filmGrainAmount * randomIntensity;

    // Apply the scaled intensity to the input color
    inputColor.rgb += scaledIntensity;
